...
parser.add_argument("--data_dir", required=True, help="Папка з .tab файлами")
parser.add_argument("--output_dir", required=True, help="Папка для збереження графіків та анімацій")
parser.add_argument("--prefix", default="pp_disk.block0.out1.", help="Префікс файлів")
parser.add_argument("--skip_rows", type=int, default=2, help="Кількість рядків заголовка для пропуску")
parser.add_argument("--fps", type=int, default=5, help="Кадрів на секунду для анімації")
...
ani.save(os.path.join(output_dir, "evolution.mp4"), fps=fps)
