...
// Disk parameters
struct Disk {Real Const; Real xm; Real k; Real Kgas; Real rho_m; Real gm; Real Re;};
Disk disk;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  disk.Re = pin->GetOrAddReal("problem", "Re", 1000.0);  // Reynolds number
  ...
  // Enroll viscosity function if Re > 0
  if (disk.Re > 0.0) {
...
// Reynolds number based viscosity
void DiskViscosity(HydroDiffusion *phdif, MeshBlock *pmb, const AthenaArray<Real> &prim,
                   const AthenaArray<Real> &bcc, int is, int ie, int js, int je,
                   int ks, int ke) {
  for (int k=ks; k<=ke; ++k) {
...
#pragma omp simd
      for (int i=is; i<=ie; ++i) {
        ...
        Real nu_tilde = (1.0/disk.Re) * (c_light/v0) * sqrt(r/(2.0*xi));
