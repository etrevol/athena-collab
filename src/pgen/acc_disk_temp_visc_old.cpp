...
Real DiskDensity(Real r) {
  ...
  if (f > 0.0) {
    Real f_center =
        0.5 - C_prime; // Maximum value of f(r) occurs exactly at r = r_center
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  r_g = 2.0 * G_GRAV * M_bh * M_SUN /
        (C_LIGHT * C_LIGHT); // Gravitational radius [cm]
  ...
  if (Globals::my_rank == 0) {
    ...
    std::cout << "  Black Hole Mass (M_bh):         " << M_bh << " M_sun"
              << std::endl;
    std::cout << "  Reference Density (rho_0):      " << rho_0 << " g/cm³"
              << std::endl;
    std::cout << "  Temperature (T_0):              " << T_0 << " K"
              << std::endl;
    std::cout << "  Mean molecular weight (mu):     " << mu_gas << std::endl;
    std::cout << "  Scaling parameter (chi):        " << chi_param << std::endl;
    std::cout << "  Calculated Sound Speed (cs_0):  " << cs0 << " cm/s"
              << std::endl;
    std::cout << "  --- Physical Scales ---" << std::endl;
    std::cout << "  Gravitational radius (r_g):     " << r_g << " cm"
              << std::endl;
    std::cout << "  Length scale (L_0):             " << L_0 << " cm"
              << std::endl;
    std::cout << "  Time scale (T_0):               " << T_scale << " s"
              << std::endl;
    std::cout << "  Mass scale:                     " << mass_scale << " M_sun"
              << std::endl;
    std::cout << "  Mdot scale:                     " << mdot_scale
              << " M_sun/yr" << std::endl;
    std::cout << "  --- Dimensionless Parameters ---" << std::endl;
    std::cout << "  Calculated Gravity (beta):      " << beta_param
              << std::endl;
    ...
    std::cout << "  Alpha Viscosity Parameter:      " << alpha_visc
              << std::endl;
  ...
  EnrollUserHistoryOutput(0, TotalDiskMass, "disk_mass",
                          UserHistoryOperation::sum);
  EnrollUserHistoryOutput(1, AccretionRate, "mdot_in",
                          UserHistoryOperation::sum);
  ...
  for (int k = ks; k <= ke; ++k) {
    for (int j = js - NGHOST; j <= je + NGHOST; ++j) {
      for (int i = is - NGHOST; i <= ie + NGHOST; ++i) {
        // Clamp global boundary lookups to avoid NaNs outside physical domain
        int ic = std::max(is, std::min(ie, i));
        Real r = pcoord->x1v(ic);
        ...
        if (need_reset) {
          ...
          Real v_phi_new = std::sqrt(beta_param / r);
...
// Spatially-varying alpha viscosity: nu = alpha * (gamma/sqrt(beta)) * (p/rho)
// * r^(3/2)
void DiskViscosity(HydroDiffusion *phdif, MeshBlock *pmb,
                   const AthenaArray<Real> &prim, const AthenaArray<Real> &bcc,
                   int is, int ie, int js, int je, int ks, int ke) {
