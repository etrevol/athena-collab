...
// Disk parameters
struct Disk {Real Const; Real xm; Real k; Real Kgas; Real rho_m; Real gm; Real alpha;};
Disk disk;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  disk.alpha = pin->GetOrAddReal("problem", "alpha", 0.01);  // Shakura-Sunyaev alpha parameter

  // Calculate the sound speed
  vs = 100000;
  v0 = vs;  // Store characteristic velocity scale
  
  // Calculate the ratio of the speed of light to the sound speed
  Crat = c_light / vs;

  // Calculate xi parameter (dimensionless scale)
  xi = disk.k;
  ...
  // Enroll viscosity function if alpha > 0
  if (disk.alpha > 0.0) {
...
#pragma omp simd
      for (int i=is; i<=ie; ++i) {
        ...
        Real nu_tilde = disk.alpha * gam * (v0/c_light) * (press/rho) * sqrt(2.0 * xi * pow(r, 3));
