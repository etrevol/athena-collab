...
// Disk parameters
struct Disk {Real Const; Real xm; Real k; Real Kgas; Real rho_m; Real gm; Real nu; Real mdot_conv;};
Disk disk;
...
Real DiskAccretionRate(MeshBlock *pmb, int iout);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  disk.nu = pin->GetOrAddReal("problem", "nu", 0.0);
  disk.mdot_conv = pin->GetOrAddReal("problem", "mdot_conv", 1.0);
  ...
  EnrollUserHistoryOutput(0, DiskAccretionRate, "M_dot");
}
...
// Newtonian Gravity and Viscosity
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        if (nu > 0.0) {
            // Left interface
            Real r_L = pmb->pcoord->x1f(i);
            Real r_i = pmb->pcoord->x1v(i);
            Real r_im1 = pmb->pcoord->x1v(i-1);
            
            Real vphi_i = prim(IVY, k, j, i);
            Real vphi_im1 = prim(IVY, k, j, i-1);
            
            Real rho_L = 0.5 * (prim(IDN, k, j, i) + prim(IDN, k, j, i-1));
            
            Real d_omega_dr_L = (vphi_i/r_i - vphi_im1/r_im1) / (r_i - r_im1);
            Real Trphi_L = rho_L * nu * r_L * d_omega_dr_L;

            // Right interface
            Real r_R = pmb->pcoord->x1f(i+1);
            Real r_ip1 = pmb->pcoord->x1v(i+1);
            
            Real vphi_ip1 = prim(IVY, k, j, i+1);
            
            Real rho_R = 0.5 * (prim(IDN, k, j, i+1) + prim(IDN, k, j, i));
            
            Real d_omega_dr_R = (vphi_ip1/r_ip1 - vphi_i/r_i) / (r_ip1 - r_i);
            Real Trphi_R = rho_R * nu * r_R * d_omega_dr_R;

            Real dx = pmb->pcoord->dx1f(i);

            // Azimuthal momentum
            Real S_phi = (1.0 / (r_i*r_i)) * ( (r_R*r_R * Trphi_R) - (r_L*r_L * Trphi_L) ) / dx;
            
            // Energy
            Real vphi_L = 0.5 * (vphi_i + vphi_im1);
            Real vphi_R = 0.5 * (vphi_ip1 + vphi_i);
            Real S_eps = (1.0 / r_i) * ( (r_R * vphi_R * Trphi_R) - (r_L * vphi_L * Trphi_L) ) / dx;

            cons(IM2, k, j, i) += dt * S_phi;
            cons(IEN, k, j, i) += dt * S_eps;
        }
      }
    }
  }
  ...
  if (pmb->pbval->block_bcs[INNER_X1] != BoundaryFlag::block) {
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        ...
        Real vr  = pmb->phydro->w(IVX, k, j, is);
