...
PROBLEM="acc_disk_temp_visc"
INPUT_TEMPLATE="inputs/hydro/athinput.acc_disk_temp_visc"
...
TESTS=(
  ...
  "nu_iso=1.0   alpha=0.0001"
  "nu_iso=1.0   alpha=0.001"
  "nu_iso=1.0   alpha=0.01"
  "nu_iso=1.0   alpha=0.1"
  "nu_iso=1.0   alpha=1.0"
  "mesh_nx1=32  mesh_nx2=32  block_nx1=16 block_nx2=16"
  "mesh_nx1=64  mesh_nx2=64  block_nx1=32 block_nx2=32"
  "mesh_nx1=64  mesh_nx2=128 block_nx1=32 block_nx2=64"
  "mesh_nx1=128 mesh_nx2=128 block_nx1=16  block_nx2=16"
  "mesh_nx1=128 mesh_nx2=128 block_nx1=32  block_nx2=32"
  "mesh_nx1=128 mesh_nx2=128 block_nx1=128 block_nx2=128"
  "mesh_x1min=0.3       mesh_x1max=4.436492"
  "mesh_x1min=0.563508  mesh_x1max=4.6"
  "mesh_x1min=0.3       mesh_x1max=7.0"
  "mesh_nx1=64  mesh_nx2=64  block_nx1=32 block_nx2=32  mesh_x1min=0.3 mesh_x1max=7.0"
  "mesh_x1min=0.1       mesh_x1max=7.0"
  "mesh_x1min=0.1       mesh_x1max=10.0"
  "mesh_x1min=0.6       mesh_x1max=4.3"
  "mesh_x1min=0.01       mesh_x1max=5.0"
  "dfloor=1.0e-4   pfloor=1.0e-6     mesh_x1min=0.573595    mesh_x1max=3.911062"
)
...
run_test() {
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    info "  [DRY RUN] Would execute: ${ATHENA_BIN} -p -i ${input_file}"
  ...
  while kill -0 "$athena_pid" 2>/dev/null || kill -0 "$pipe_pid" 2>/dev/null; do
    ...
    if [[ -n "$eta_str" ]]; then
      ...
      # ── 2. Fallback: no ETA yet (early phase) – check for any new output ──
      local cur_lines
...
if [[ "$DRY_RUN" -eq 0 && ! -x "${ATHENA_BIN}" ]]; then
  err "Binary '${ATHENA_BIN}' not found or not executable."
...
for i in "${!TESTS[@]}"; do
  ...
  run_test "$test_num" "$test_dir" "$local_input"
  popd > /dev/null
