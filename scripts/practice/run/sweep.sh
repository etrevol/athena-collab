...
#    sim_time stops advancing for longer than HANG_TIMEOUT seconds the
...
# Build-time defaults (compile-time Riemann solver and ghost-cell count).
# Tests that do not specify b_flux / b_nghost use the default binary.
DEFAULT_FLUX="hllc"
DEFAULT_NGHOST="2"
CONFIGURE_BASE_FLAGS="--prob=${PROBLEM} --coord=cylindrical"

# Hang detection
HANG_TIMEOUT=90          # seconds: kill if sim_time has not advanced
...
#
#  Build-time key prefixes (b_*) select a pre-built binary; they are NOT
#  written to the input file.  All other keys are applied to the input file.
#
#    b_flux=VALUE   → bin/athena_{VALUE}_{nghost}   (--flux configure flag)
#    b_nghost=N     → bin/athena_{flux}_{N}         (--nghost configure flag)
#
#  Default binary (when no b_* keys): bin/athena  (hllc, nghost=2)
#
TESTS=(
  "nu_iso=0.0   alpha=0.0"
  "nu_iso=1.0   alpha=0.001"
  "nu_iso=1.0   alpha=0.01"
)
...
# Walk up from this script until we find the repository root, so that moving the
# script around the materials/ tree does not silently break every path below.
REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
while [[ "${REPO_DIR}" != "/" && ! -f "${REPO_DIR}/configure.py" ]]; do
  REPO_DIR="$(dirname "${REPO_DIR}")"
done
[[ -f "${REPO_DIR}/configure.py" ]] || { echo "sweep.sh: cannot locate repository root" >&2; exit 1; }
...
# ── BINARY SELECTION ──────────────────────────────────────────────────────────
#
# Compile-time parameters (b_flux, b_nghost) select a specific binary.
# Convention: bin/athena                    → default (hllc, nghost=2)
#             bin/athena_{flux}_{nghost}   → any other combination
#
# get_binary FLUX NGHOST → absolute path
get_binary() {
  local flux="$1" nghost="$2"
  if [[ "$flux" == "$DEFAULT_FLUX" && "$nghost" == "$DEFAULT_NGHOST" ]]; then
    echo "${REPO_DIR}/bin/athena"
  else
    echo "${REPO_DIR}/bin/athena_${flux}_${nghost}"
  fi
}

# ensure_binary FLUX NGHOST → exits non-zero if unavailable and can't build
ensure_binary() {
  local flux="$1" nghost="$2"
  local bin_path
  bin_path=$(get_binary "$flux" "$nghost")

  if [[ -x "$bin_path" ]]; then
    info "  Binary: $(basename "$bin_path")  ✓"
    return 0
  fi

  warn "  Binary '${bin_path}' not found – attempting to compile…"
  local build_log="${SWEEP_DIR}/build_${flux}_ng${nghost}.log"
  (
    cd "${REPO_DIR}"
    python3 configure.py ${CONFIGURE_BASE_FLAGS} --flux="${flux}" --nghost="${nghost}" \
      > "$build_log" 2>&1 && \
    make clean >> "$build_log" 2>&1 && \
    make -j"$(nproc)" >> "$build_log" 2>&1 && \
    cp bin/athena "$bin_path"
  )
  local rc=$?
  if [[ $rc -ne 0 ]]; then
    err "  Build FAILED for flux=${flux} nghost=${nghost}. See ${build_log}"
    return 1
  fi
  info "  Built → ${bin_path}"
}
...
#   • if sim_time has not advanced for HANG_TIMEOUT seconds → kill
...
run_test() {
  ...
  while kill -0 "$athena_pid" 2>/dev/null || kill -0 "$pipe_pid" 2>/dev/null; do
    ...
    # ── 1. PRIMARY: sim_time progress ─────────────────────────────────────
    # A real hang here is a timestep collapse (dt -> ~1e-88): the model time
    # freezes completely while cycles keep spinning.  ETA must NOT be used for
    # this.  On a long run the ETA legitimately stays flat or rises for minutes
    # while dt drifts slowly down, and a transient dt dip that the run fully
    # recovers from also pushes it up.  Both killed healthy runs in
    # sweep-20260728-091727: t3 died after 2:53 with a perfectly smooth dt
    # (5.27e-6 -> 5.23e-6), and t2 died inside a dip it recovered from -- a
    # bit-identical run outside the sweep went on to finish all 30 orbits.
...
generate_report() {
  ...
  local -a t_names t_params t_statuses t_durations t_cycles t_simtimes t_binaries
  ...
  for tdir in "${test_dirs[@]}"; do
    ...
    local binused="N/A"
    [[ -f "${tdir}/bin.txt" ]] && binused=$(basename "$(cat "${tdir}/bin.txt")")
    t_binaries+=("$binused")
  ...
  # ── write report ──────────────────────────────────────────────────────────
  {
    ...
    echo "| **Default binary** | \`bin/athena\` (hllc, nghost=2) |"
    ...
    echo "## Summary Table"
    echo ""
    echo "| \# | Test directory | Parameters | Binary | Status | Duration | Cycles | Sim time |"
    echo "|---|---|---|---|---|---|---|---|"
...
if [[ "$DRY_RUN" -eq 0 && ! -x "${ATHENA_BIN}" ]]; then
  err "Default binary '${ATHENA_BIN}' not found or not executable."
  err "Please build first:  bash build.sh"
  err "(Per-test binaries will be built on demand if missing.)"
  exit 1
fi
...
for i in "${!TESTS[@]}"; do
  ...
  test_nghost="$DEFAULT_NGHOST"
  for _kv in $params; do
    case "${_kv%%=*}" in
      b_flux)   test_flux="${_kv#*=}"   ;;
      b_nghost) test_nghost="${_kv#*=}" ;;
    esac
  done
  test_bin=$(get_binary "$test_flux" "$test_nghost")
  echo "$test_bin" > "${test_dir}/bin.txt"

  if [[ "$DRY_RUN" -eq 0 ]]; then
    ensure_binary "$test_flux" "$test_nghost" || {
      err "Skipping test ${test_num} – binary unavailable."
