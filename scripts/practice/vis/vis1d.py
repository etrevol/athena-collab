...
def read_athena_1d_single_block(filename):
    """Read single block of 1D Athena++ file"""
    ...
    # Columns: i, x1v(r), rho, press, vel1(r), vel2(phi), vel3(z)
    return {
        'time': time,
        'cycle': cycle,
        'r': data[:, 1],
...
def read_athena_1d(file_list):
    """Read and combine 1D Athena++ files from multiple blocks
    
    Args:
        file_list: list of file paths (one per block) or single file path
    
    Returns:
        dict with combined radial profile data
    """
    ...
    for filename in file_list:
        block_data = read_athena_1d_single_block(filename)
    ...
    # Sort by radius (blocks may not be in radial order)
    ...
    result = {
        ...
        'cycle': cycle,
        'r': all_r[sort_idx],
        'density': all_rho[sort_idx],
        'pressure': all_press[sort_idx],
        'vel_r': all_vel_r[sort_idx],
        'vel_phi': all_vel_phi[sort_idx],
        'vel_z': all_vel_z[sort_idx],
    }
...
def plot_radial_profiles(data, frame_num, output_dir):
    ...
    fig.suptitle(f"Radial Profiles (t={data['time']:.3f}, cycle={data['cycle']})", 
...
def create_radial_evolution_animation(available_frames, frames_dict, output_dir):
    ...
    # Create figure
    fig, axes = plt.subplots(2, 2, figsize=CONFIG['figsize'])
    fig.suptitle("Radial Profile Evolution", fontsize=16, fontweight='bold', y=0.98)
