...
def discover_uov_files(data_dir):
    """Scan data_dir for uov-type .tab files (forces output).
    
    We identify uov files by checking the header for 'f_grav' or by
    using the output number: collects all .tab files, groups them by
    (output_number, frame), then picks the output group that contains
    uov data (4 uov columns after the standard 4 coordinate columns).
    
    Returns:
        frames_dict: {frame_int: [sorted list of file paths]}
        base_name: str
        num_blocks: int
    """
    ...
    # Group all tab files by output-number
    # key: out_num -> {frame: [(block, path), ...]}
    from collections import defaultdict
    by_outnum = defaultdict(lambda: defaultdict(list))
    base_names = set()

    for f in all_files:
        parsed = parse_uov_filename(f)
        if parsed is None:
            continue
        base, block, frame, is_mb = parsed
        base_names.add(base)
        # Determine output number from filename
        m_mb = re.match(r'.+\.block\d+\.out(\d+)\.\d+\.tab$', f)
        m_sb = re.match(r'.+\.out(\d+)\.\d+\.tab$', f)
        m_out = m_mb or m_sb
        out_num = int(m_out.group(1)) if m_out is not None else 0
        block_idx = block if block is not None else 0
        by_outnum[out_num][frame].append((block_idx, os.path.join(data_dir, f)))

    if not by_outnum:
        return {}, 'unknown', 0

    # Pick the output number whose files contain uov force data.
    # Heuristic: read a sample file's header and count columns,
    # or look for 8-column files (i, r, j, phi, f_grav, f_centr, f_press, f_sum).
    chosen_out = None
    for out_num, frames in by_outnum.items():
        if not frames:
            continue
        sample_frame = next(iter(frames.values()))
        sample_path = sample_frame[0][1]
        try:
            ncols = _count_data_columns(sample_path)
            if ncols == 8:  # 4 coords + 4 force columns
                chosen_out = out_num
                break
        except Exception:
            pass
    ...
    if chosen_out is None:
        # Fallback: pick the output with most columns
        def max_cols(frames):
            try:
                p = next(iter(frames.values()))[0][1]
                return _count_data_columns(p)
            except Exception:
                return 0
        chosen_out = max(by_outnum.keys(), key=lambda n: max_cols(by_outnum[n]))

    frames_raw = by_outnum[chosen_out]
    frames_dict = {}
    max_block = 0
    for frame, block_list in frames_raw.items():
        block_list.sort(key=lambda x: x[0])
        max_block = max(max_block, max(b for b, _ in block_list))
        frames_dict[frame] = [path for _, path in block_list]

    base_name = list(base_names)[0] if base_names else 'unknown'
    num_blocks = max_block + 1
    return frames_dict, base_name, num_blocks
...
def read_uov_frame(file_list):
    """Read and merge multi-block uov data for one frame.
    
    Returns dict with 2D arrays shaped (nphi, nr) and coordinate arrays.
    """
    if isinstance(file_list, str):
        file_list = [file_list]

    blocks = [read_uov_single_block(f) for f in file_list]
    ...
    all_fg = np.concatenate([b['f_grav'] for b in blocks])
    all_fc = np.concatenate([b['f_centr'] for b in blocks])
    all_fp = np.concatenate([b['f_press'] for b in blocks])
    all_fs = np.concatenate([b['f_sum'] for b in blocks])
    ...
    f_grav_2d = np.zeros((nphi, nr))
    f_centr_2d = np.zeros((nphi, nr))
    f_press_2d = np.zeros((nphi, nr))
    f_sum_2d = np.zeros((nphi, nr))

    for n in range(len(all_r)):
        ri = r_to_idx[all_r[n]]
        pi = phi_to_idx[all_phi[n]]
        f_grav_2d[pi, ri] = all_fg[n]
        f_centr_2d[pi, ri] = all_fc[n]
        f_press_2d[pi, ri] = all_fp[n]
        f_sum_2d[pi, ri] = all_fs[n]
...
def plot_sum_frame(data, frame_num, output_dir):
    ...
    fig.suptitle(
        f"Net Radial Force (φ-averaged) — t = {data['time']:.4f}, cycle = {data['cycle']}",
        fontsize=13, fontweight='bold'
    )
