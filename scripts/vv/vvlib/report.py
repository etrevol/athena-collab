...
def _tex(x, digits=3):
    """A float as LaTeX maths, in scientific form when the magnitude warrants it."""
    if x is None or not np.isfinite(x):
        return "---"
    if x == 0.0:
        return "$0$"
    if 0.1 <= abs(x) < 1e4:                    # below this a table reads better in
        return f"${x:.{digits}f}$"             # scientific form, and mixing the two
    if abs(x) >= 1e4:                          # inside one column reads worst of all
        return f"${x:.{digits}f}$"
    m, e = f"{x:.{digits - 1}e}".split("e")
    return f"${m} \\times 10^{{{int(e)}}}$"


def write_tex(suite, model, checks, data, path=None):
    """The result tables of the formulary section, generated from the run that produced
    them. They used to be transcribed by hand, which is a silent-drift problem: the
    document keeps compiling with numbers from a superseded suite. Emitting them means a
    stale table is a diff, not a discovery."""
    by_name = {c.name: c for c in checks}

    def value(name):
        c = by_name.get(name)
        return None if c is None or c.value is None else float(c.value)

    # Each table is a macro rather than plain text, so the document keeps deciding where
    # its tables sit and what prose goes between them; \input only supplies the numbers.
    L = ["% Generated by scripts/vv/run_vv.py - do not edit.",
         f"% Suite: {suite.dir.name}",
         f"% Commit: {_provenance_sha(suite)}",
         "% Defines \\vvtable{static,resid,ring,ppi} for the full document and",
         "% \\vvctable{...} for the concise one - same numbers, captionless.",
         ""]
    add = L.append
    ...
    # -- static verification -------------------------------------------------
    static_rows = [
        ("Bernoulli integral constant across the torus", "bernoulli",
         r"Eq.~(\ref{therm:eq:bernoulli_p_rho})"),
        ("$f(r_{\\text{in}}) = f(r_{\\text{out}}) = 0$", "surfaces",
         r"Eq.~(\ref{therm:eq:surface})"),
        ("$H/r = \\sqrt{(\\gamma-1)(0.5-C')}$", "H_over_r", "---"),
        ("$P_{\\text{orb}}$, $(p/\\rho)_{\\text{phys}}$ over "
         "$T_0 = 5\\times10^3 \\dots 5\\times10^5$~K", "units_T0",
         r"Eq.~(\ref{vv:eq:pr_phys})"),
        ("Generator against the analytic module", "pgen_vs_theory", "---"),
    ]
    begin("vvtablestatic")
    add(r"\begin{table}[!ht]")
    add(r"\centering")
    add(r"\begin{tabular}{lll}")
    add(r"\toprule")
    add(r"Quantity & Measured deviation & Reference \\")
    add(r"\midrule")
    for label, name, ref in static_rows:
        add(f"{label} & {_tex(value(name), 2)} & {ref} \\\\")
    add(r"\bottomrule")
    add(r"\end{tabular}")
    add(r"\caption{Static verification. The fourth row is the numerical demonstration of")
    add(r"Conclusion~1: the physical period and thermal state do not move when the "
        r"thermal unit is changed by two orders of magnitude.}")
    add(r"\label{vv:tab:static}")
    add(r"\end{table}")
    end()

    # -- discrete residual ---------------------------------------------------
    levels = data.get("resid_levels", [])
    zones = data.get("resid_zones", {})
    if levels and zones:
        p_core, p_surf = data.get("resid_order", (np.nan, np.nan))
        begin("vvtableresid")
        add(r"\begin{table}[!ht]")
        add(r"\centering")
        add(r"\begin{tabular}{l" + "c" * len(levels) + "}")
        add(r"\toprule")
        add("Zone & " + " & ".join(f"$N = {n}$" if i == 0 else f"${n}$"
                                   for i, n in enumerate(levels)) + r" \\")
        add(r"\midrule")
        for key, label in (("core", "Torus core (smooth)"),
                           ("surface", "Torus surface ($C^3$)"),
                           ("funnel", "Inner funnel (boundary)"),
                           ("outer", "Outer ambient (balanced)")):
            if key in zones:
                add(f"{label:28s} & "
                    + " & ".join(_tex(v) for v in zones[key]) + r" \\")
        add(r"\bottomrule")
        add(r"\end{tabular}")
        add(r"\caption{Relative residual $\mathcal{E}_N$ of Eq.~(\ref{vv:eq:resid}). "
            r"The observed order from Eq.~(\ref{vv:eq:order_resid}) on the finest pair "
            f"is $p = {p_core:.2f}$ in the core and $p = {p_surf:.2f}$ at the surface. "
            r"The ambient does not converge because it is already exact, as "
            r"Conclusion~3 requires.}")
        add(r"\label{vv:tab:resid}")
        add(r"\end{table}")
        end()

    # -- ring ----------------------------------------------------------------
    rl = data.get("ring_levels", [])
    nus = data.get("ring_nu_eff", [])
    nums = data.get("ring_numerical", {})
    if rl and nus:
        import importlib
        cfg = importlib.import_module("vvlib.cases")
        begin("vvtablering")
        add(r"\begin{table}[!ht]")
        add(r"\centering")
        add(r"\begin{tabular}{l" + "c" * len(rl) + "}")
        add(r"\toprule")
        add("Ring resolution & " + " & ".join(f"$N = {n}$" if i == 0 else f"${n}$"
                                              for i, n in enumerate(rl)) + r" \\")
        add(r"\midrule")
        add(r"$\nu_{\text{eff}}$ from Eq.~(\ref{vv:eq:nu_eff}), applied "
            f"$\\nu = {_tex(cfg.RING_NU)[1:-1]}$ & "
            + " & ".join(_tex(v, 4) for v in nus) + r" \\")
        add("Deviation & "
            + " & ".join(f"${100 * (v / cfg.RING_NU - 1):+.2f}\\%$" for v in nus)
            + r" \\")
        if nums:
            add(r"\midrule")
            order = [n for n in rl if n in nums]
            add(r"$\nu_{\text{num}}$ bound, $\nu = 0$ run & "
                + " & ".join(_tex(nums[n]["bound"], 2) for n in order) + r" \\")
            add(r"Equivalent $\alpha_{\text{num}}$ & "
                + " & ".join(_tex(_alpha_eq(nums[n]["bound"], model), 2)
                             for n in order) + r" \\")
        add(r"\bottomrule")
        add(r"\end{tabular}")
        add(r"\caption{Lynden-Bell \& Pringle ring. The viscous operator reproduces an "
            r"exact solution to $0.4\%$. The numerical viscosity is bounded, not "
            r"resolved: its drift stays at the level of the fit scatter at every "
            r"resolution, though the bound itself falls by a factor of four per "
            r"refinement, as truncation error should.}")
        add(r"\label{vv:tab:ring}")
        add(r"\end{table}")
        end()

    # -- growth rates --------------------------------------------------------
    theory = data.get("ppi_theory", {})
    meas = data.get("ppi_measured", {})
    if theory and meas:
        begin("vvtableppi")
        add(r"\begin{table}[!ht]")
        add(r"\centering")
        ...
        add(r"\toprule")
        add(r"Mode & Theory, Eq.~(\ref{vv:eq:linearised}) & Simulation & Deviation "
            r"& $R^2$ \\")
        add(r"\midrule")
        for cid, label in (("ppi_m1", "$m = 1$"), ("ppi_m2", "$m = 2$"),
                           ("ppi_m3", "$m = 3$"),
                           ("ppi_m2_small", r"$m = 2$, seed $\times 0.1$")):
            if cid not in meas:
                continue
            rec = meas[cid]
            th = theory[rec["m"]]["rate"] * model.P_orb
            fit = rec["fit"]
            add(f"{label} & ${th:.4f}$ & ${fit['rate']:.4f}$ & "
                f"${100 * abs(fit['rate'] - th) / th:.1f}\\%$ & "
                f"${fit['r2']:.4f}$ \\\\")
        add(r"\bottomrule")
        add(r"\end{tabular}")
        add(r"\caption{Growth rates in e-foldings per orbital period at $\tilde{R}_0$. "
            r"The eigenvalue problem is solved independently of the simulation and is "
            r"converged under refinement of its own grid. All three modes corotate "
            r"inside the torus.}")
        add(r"\label{vv:tab:ppi}")
        add(r"\end{table}")
        end()

    # -- concise variants: same numbers, no captions, shorter headers ---------
    # The concise document carries the same four tables in its own style. Rendering them
    # from the same locals is what keeps the two documents from disagreeing about a
    # measurement, which is the whole point of generating them.
    begin("vvctablestatic")
    add(r"\begin{table}[!ht]")
    add(r"\centering")
    add(r"\begin{tabular}{ll}")
    add(r"\toprule")
    add(r"Quantity & Deviation \\")
    add(r"\midrule")
    for label, name, _ in static_rows:
        add(f"{label} & {_tex(value(name), 2)} \\\\")
    add(r"\bottomrule")
    add(r"\end{tabular}")
    add(r"\end{table}")
    end()
    ...
    if levels and zones:
        ...
        add(r"\begin{tabular}{l" + "c" * len(levels) + "}")
        add(r"\toprule")
        add("Zone & " + " & ".join(f"${n}$" for n in levels) + r" \\")
        add(r"\midrule")
        for key, label in (("core", "Torus core"), ("surface", "Torus surface"),
                           ("funnel", "Inner funnel"), ("outer", "Outer ambient")):
            if key in zones:
                add(f"{label:16s} & " + " & ".join(_tex(v) for v in zones[key]) + r" \\")
        add(r"\bottomrule")
        add(r"\end{tabular}")
        add(r"\end{table}")
        end()
    ...
    if rl and nus:
        ...
        add(r"\begin{tabular}{l" + "c" * len(rl) + "}")
        add(r"\toprule")
        add("Ring resolution & " + " & ".join(f"${n}$" for n in rl) + r" \\")
        add(r"\midrule")
        add(r"$\nu_{\text{eff}}$ (applied " + f"${_tex(cfg.RING_NU)[1:-1]}$) & "
            + " & ".join(_tex(v, 4) for v in nus) + r" \\")
        add("Deviation & "
            + " & ".join(f"${100 * (v / cfg.RING_NU - 1):+.2f}\\%$" for v in nus)
            + r" \\")
        if nums:
            ...
            add(r"$\nu_{\text{num}}$ bound ($\nu = 0$) & "
                + " & ".join(_tex(nums[n]["bound"], 2) for n in order) + r" \\")
            add(r"$\alpha_{\text{num}}$ equivalent & "
                + " & ".join(_tex(_alpha_eq(nums[n]["bound"], model), 2)
                             for n in order) + r" \\")
        add(r"\bottomrule")
        add(r"\end{tabular}")
    ...
    if theory and meas:
        ...
        add(r"\begin{tabular}{lcccc}")
        add(r"\toprule")
        add(r"Mode & Theory & Simulation & Deviation & $R^2$ \\")
        add(r"\midrule")
        for cid, label in (("ppi_m1", "$m = 1$"), ("ppi_m2", "$m = 2$"),
                           ("ppi_m3", "$m = 3$"),
                           ("ppi_m2_small", r"$m = 2$, seed $\times0.1$")):
            if cid not in meas:
                continue
            rec = meas[cid]
            th = theory[rec["m"]]["rate"] * model.P_orb
    ...
    text = "\n".join(L)
    (suite.dir / "vv_tables.tex").write_text(text)
    if path:
        import pathlib
...
def _alpha_eq(bound, model):
    from . import ring as ring_mod
    try:
        return ring_mod.alpha_equivalent(bound, model)
    except Exception:
        return float("nan")


def _provenance_sha(suite):
    try:
        return json.loads((suite.materials / "provenance.json").read_text())["git_sha"][:12]
    except Exception:
        return "unknown"
