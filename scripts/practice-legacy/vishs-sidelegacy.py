...
OPTIONS:
    --mode {default,all,custom,force_animation}
        ...
        force_animation  - frame-by-frame animation of grav_force vs press_force
    ...
    --fps N               - FPS for force_animation video (default: 10)
    --anim_subsample N    - use every Nth time step for animation (default: 1)
...
VARIABLE_INFO = {
    ...
    'grav_force':  {'label': r'Gravitational Force $F_\mathrm{grav}$ [code units]',  'unit': 'code',           'color': 'crimson'},
    'press_force': {'label': r'Pressure Gradient Force $F_{\nabla P}$ [code units]', 'unit': 'code',           'color': 'royalblue'},
}
...
parser.add_argument("--mode", default="default",
                    choices=['default', 'all', 'custom', 'force_animation'],
...
parser.add_argument("--fps", type=int, default=10,
                    help="FPS for force_animation video (default: 10)")
parser.add_argument("--anim_subsample", type=int, default=1,
                    help="Use every Nth time step for animation frames (default: 1)")
...
elif args.mode == 'force_animation':
    vars_to_plot = ['grav_force', 'press_force']
    print(f"Force animation mode: will animate {', '.join(vars_to_plot)}")
else:
...
def plot_variable(time, var_data, var_name, output_dir, linear_scale=False):
    ...
    ax.set_title(f'{label} Evolution', fontsize=14, fontweight='bold', pad=15)
...
def plot_disk_mass_and_mdot(time, disk_mass, mdot_in, output_dir):
    ...
    # Title
    ax1.set_title('Disk Mass and Accretion Rate Evolution', 
...
def create_force_animation(time, grav_force, press_force, output_dir,
                            fps=10, subsample=1):
    """Create frame-by-frame animation of radial force balance.

    Generates sequential PNG frames (force_balance_frame_*****.png) following
    the same pattern as radial_profile_frame_*****.png in vis2d.py, then
    assembles them into an MP4 video if imageio is available.

    At each frame i the plot shows grav_force and press_force as growing
    time-series curves from t=0 up to t=time[i] on the same axes.

    Args:
        time:        1-D array of simulation time values
        grav_force:  1-D array of integrated gravitational force [code units]
        press_force: 1-D array of integrated pressure-gradient force [code units]
        output_dir:  directory to save frames and final video
        fps:         frames per second for the assembled video
        subsample:   step between selected time indices (1 = every time step)
    """
    indices = np.arange(0, len(time), subsample)
    n_frames = len(indices)
    print(f"  Total time steps : {len(time)}")
    print(f"  Animation frames : {n_frames} (every {subsample} step(s))")

    # Pre-compute fixed y-axis limits from the full dataset so the axes never
    # jump between frames, making it easy to compare forces visually.
    all_vals = np.concatenate([grav_force, press_force])
    finite = all_vals[np.isfinite(all_vals)]
    if len(finite) > 0:
        y_min, y_max = float(np.min(finite)), float(np.max(finite))
        pad = 0.08 * (y_max - y_min) if y_max > y_min else abs(y_min) * 0.1 + 1e-30
        y_lim = (y_min - pad, y_max + pad)
    else:
        y_lim = (-1.0, 1.0)

    grav_info  = VARIABLE_INFO.get('grav_force',
                     {'label': r'Gravity $F_\mathrm{grav}$',   'color': 'crimson'})
    press_info = VARIABLE_INFO.get('press_force',
                     {'label': r'Pressure $F_{\nabla P}$',     'color': 'royalblue'})

    saved_frames = []
    report_every = max(1, n_frames // 10)

    print("  Rendering frames...")
    for frame_idx, t_idx in enumerate(indices):
        fig, ax = plt.subplots(figsize=CONFIG['figsize'], dpi=CONFIG['dpi'])

        # Grow curves up to the current time index
        t_sl    = time[:t_idx + 1]
        grav_sl = grav_force[:t_idx + 1]
        pres_sl = press_force[:t_idx + 1]

        ax.plot(t_sl, grav_sl,
                color=grav_info['color'], linewidth=CONFIG['linewidth'],
                alpha=CONFIG['alpha'], label=grav_info['label'])
        ax.plot(t_sl, pres_sl,
                color=press_info['color'], linewidth=CONFIG['linewidth'],
                alpha=CONFIG['alpha'], label=press_info['label'])

        # Thin vertical line marking the current time
        ax.axvline(time[t_idx], color='gray', linewidth=0.8,
                   linestyle='--', alpha=0.5)

        ax.set_xlim(time[0], time[-1])
        ax.set_ylim(y_lim)
        ax.set_xlabel('Time', fontsize=12, fontweight='bold')
        ax.set_ylabel('Integrated radial force [code units]',
                      fontsize=12, fontweight='bold')
        ax.set_title(
            f'Radial Force Balance  (t = {time[t_idx]:.4e})',
            fontsize=14, fontweight='bold', pad=15
        )
        ...
        frame_file = os.path.join(
            output_dir, f'force_balance_frame_{frame_idx:05d}.png')
        plt.savefig(frame_file, dpi=CONFIG['dpi'], bbox_inches='tight')
        plt.close()
        saved_frames.append(frame_file)

        if (frame_idx + 1) % report_every == 0:
            print(f"    [{frame_idx + 1:{width}d}/{n_frames}]  t = {time[t_idx]:.4e}")

    print(f"  Saved {len(saved_frames)} frames to: {output_dir}")

    # Assemble into MP4 using imageio (optional dependency)
    ...
    try:
        ...
        writer.close()
        print(f"  Assembled video  : {video_file}")
    except ImportError:
...
if args.mode == 'default':
    ...
    required = ['grav_force', 'press_force']
    missing = [v for v in required if v not in data]
    if missing:
        print(f"  ✗ ERROR: Variables not found in .hst: {', '.join(missing)}")
        print(f"  Available columns: {', '.join(all_vars)}")
        print("  Rebuild with acc_disk_visc.cpp that has RadialGravForce / RadialPressForce.")
        sys.exit(1)
    print("\nCreating force balance animation...")
    saved = create_force_animation(
        time, data['grav_force'], data['press_force'],
        output_dir, fps=args.fps, subsample=args.anim_subsample
    )
